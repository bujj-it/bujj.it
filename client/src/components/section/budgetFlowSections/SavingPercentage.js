...
const Expenses = props => {
  ...
  const savingPercentageCards = availableSavingPercentages.map(percentage => {
    return <SavingPercentageCard savingPercentage={percentage} key={percentage} />;
  });
      ...
      >
        ...
        <div className="saving-percentages-selector">
          ...
          <div className="toggle-switch-container flex-center">
            <button type="button" className="toggle-switch flex-center" onClick={handleToggleClick}>
              {toggleText}
            </button>
          </div>
        </div>
